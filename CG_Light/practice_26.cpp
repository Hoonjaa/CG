...
const glm::vec3 lightInitialPos = glm::vec3(0.0f, 0.0f, 2.0f);


//Cube* cube = nullptr; 예시임 포인터로 객체 선언
// 포인터로 하는 이유는 셰이더가 만들어지는 등 기본 세팅 코드가 먼저 작동해야 객체 생성가능
// 따라서 main 함수 안에서 new로 객체 생성
Planet* planet_red = nullptr;
Planet* planet_green = nullptr;
Planet* planet_blue = nullptr;
...
void main(int argc, char** argv)										//--- 윈도우 출력하고 콜백함수 설정
{
	...
	// --------------기본 객체 생성 시 여기서 작업-------------
	planet_red = new Planet(1.0f, 36, 18, glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 0.0f)); // 빨간색 행성 (0, 0, 0)
	planet_green = new Planet(1.0f, 36, 18, glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(-3.0f, 0.0f, -3.0f)); // 초록색 행성 (-2, -2, 0)
	planet_blue = new Planet(1.0f, 36, 18, glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(-6.0f, 0.0f, -6.0f)); // 파란색 행성 (-4, -4, 0)
...
GLvoid drawScene()														//--- 콜백 함수: 그리기 콜백 함수
{
	...
	planet_red->draw(shaderProgramID, glm::mat4(1.0f), Transform_matrix);
	planet_green->draw(shaderProgramID, glm::mat4(1.0f), Transform_matrix);
	planet_blue->draw(shaderProgramID, glm::mat4(1.0f), Transform_matrix);
...
GLvoid Keyboard(unsigned char key, int x, int y)
{
	...
	case 'R':
		...
		rotation = glm::rotate(glm::mat4(1.0f), glm::radians(lightOrbitAngle), glm::vec3(0.0f, 1.0f, 0.0f));
		LightPos = glm::vec3(rotation * glm::vec4(lightInitialPos, 1.0f));
		break;
	case 'c':
